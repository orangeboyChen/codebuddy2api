...
コンソールは PWA なので、ブラウザからインストールできます。インストールしたアイコンからは Dashboard が直接開きます。

- デスクトップの Chrome / Edge:アドレスバー右側のインストールアイコン、またはメニューの「CodeBuddy2API をインストール」。
- iOS Safari:共有 → ホーム画面に追加。
- インストールは HTTPS、または localhost でのみ提供されます。
- インストールしてもショートカットが増えるだけです。ゲートウェイが停止しているとブラウザのオフライン画面が表示されます。
